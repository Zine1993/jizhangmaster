...
// 头像/昵称工具
function _hashCode(str: string): number { let h = 0; for (let i = 0; i < str.length; i++) { h = (h << 5) - h + str.charCodeAt(i); h |= 0; } return Math.abs(h); }
...
function _contrastOn(hex: string): '#fff' | '#111' { const v=hex.replace('#',''); const r=parseInt(v.substring(0,2),16), g=parseInt(v.substring(2,4),16), b=parseInt(v.substring(4,6),16); const lum=(0.2126*r+0.7152*g+0.0722*b)/255; return lum<0.6?'#fff':'#111'; }
function pickAvatarColors(seed: string){ const h=_hashCode(seed)%360; const bg=_hslToHex(h,65,55); const fg=_contrastOn(bg); return { bg, fg }; }
function pickInitial(seed: string){ const m=seed.match(/[A-Za-z0-9]/); return (m?m[0]:'U').toUpperCase(); }
...
export default function SettingsScreen() {
  ...
  const lockRotate = lockAnim.interpolate({ inputRange: [0, 0.5, 1], outputRange: ['0deg', '-20deg', '0deg'] });
  const lockScale = lockAnim.interpolate({ inputRange: [0, 0.5, 1], outputRange: [1, 1.15, 1] });
  const lockOpacity = lockAnim.interpolate({ inputRange: [0, 0.4, 0.5, 1], outputRange: [1, 1, 0, 0] });
  const unlockOpacity = lockAnim.interpolate({ inputRange: [0, 0.4, 0.5, 1], outputRange: [0, 0, 1, 1] });
          ...
          <View style={[styles.modalSheet, { backgroundColor: colors.surface }]}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 12, marginTop: 8 }}>
              ...
              <Button
                ...
                onPress={async () => {
                  try {
                    if (!setupPwd || !setupPwd2) { Alert.alert(t('tip'), t('fillAllFields')); return; }
                    if (setupPwd !== setupPwd2) { Alert.alert(t('tip'), t('passwordNotMatch')); return; }
                    if (setupPwd.length < 4) { Alert.alert(t('tip'), t('passwordTooShort')); return; }
...
const styles = StyleSheet.create({
  ...
  profileName: { fontSize: 18, fontWeight: 'bold', flexShrink: 1 },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 16 },
  rowLeft: { flexDirection: 'row', alignItems: 'center', gap: 16 },
  iconBg: { width: 36, height: 36, borderRadius: 8, justifyContent: 'center', alignItems: 'center' },
  rowTitle: { fontSize: 16, fontWeight: '500' },
  rowRight: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  rowValue: { fontSize: 14 },
  separator: { height: StyleSheet.hairlineWidth, marginLeft: 68 },
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', padding: 24 },
  modalSheet: { borderRadius: 12, padding: 16, width: '90%', alignSelf: 'center', shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 8 },
  modalTitle: { fontSize: 16, fontWeight: '700', marginBottom: 8 },
  modalOption: { paddingVertical: 12, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderRadius: 8, borderWidth: StyleSheet.hairlineWidth, borderColor: '#00000020', marginTop: 8 },
  modalOptionLeft: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  languageBadge: { width: 22, height: 22, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
  modalCancel: { paddingVertical: 12, alignItems: 'center', marginTop: 4, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#00000020' },
  input: { borderWidth: StyleSheet.hairlineWidth, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, fontSize: 16, marginTop: 8 },
});